...
###################################################################################################################
# Adapted by: Cristina Almagro Perez, 2022. ETH Zurich.
#------------------------------------------------------------------------------------------------------------------
# Contributions:
# Contributions: created code of the consolidation of segmentation masks through averaging over several epochs,
#  test time augmentations and overlapping patches. Developed function merge_pred_seg.
...
def load_dice(checkpoint_path):
    dice_checkpoint = np.load(os.path.join(checkpoint_path, 'dice_per_epoch.npy'))
    return dice_checkpoint
...
def prepare_monitoring(cf):
    ...
    # generate instance of monitor plot class.
...
def create_csv_output(results_list, cf, logger):
    ...
    for r in results_list:
        ...
        for bix, box in enumerate(r[0][0]):
            ...
            score = box['box_score']  # here the score is the box score
            print(score)
            pred_class_id = box['box_pred_class_id']
            out_coords = []
            if score >= cf.min_det_thresh:
                print("Box score greater than 0.9")
...
def merge_pred_seg(cf, logger):
    """
      Specify folders to merge the resulting predictions
       """
    folds = cf.folds
    outpth = os.path.join(cf.exp_dir, 'test_pred_seg')
    if not os.path.exists(outpth):
        print("Creating outpth")
        os.makedirs(outpth)

    logger.info('Saving combined metastases at {}'.format(outpth))

    # Load file from the first fold
    fold_name = 'fold_' + str(folds[0])
    pth = os.path.join(cf.exp_dir, fold_name)
    name_seg = 'raw_pred_boxes_&_seg_hold_out_list_'+ str(cf.ap_match_ious[0]) + '.pickle'
    #datafile = os.path.join(pth, 'raw_pred_boxes_&_seg_hold_out_list.pickle')
    datafile = os.path.join(pth, name_seg)
    all_segs = pd.read_pickle(datafile)  # list
    idx = [all_segs[batch_instance][1] for batch_instance in range(len(all_segs))]  # patients id


    for count,pid in enumerate(idx):

        total_mask = np.zeros(all_segs[0][0].shape)
        for ifold in folds:
            # Load file with segmentations
            name = 'fold_' + str(ifold)
            pth = os.path.join(cf.exp_dir, name)
            name_seg = 'raw_pred_boxes_&_seg_hold_out_list_' + str(cf.ap_match_ious[0]) + '.pickle'
            #datafile = os.path.join(pth, 'raw_pred_boxes_&_seg_hold_out_list.pickle')
            datafile = os.path.join(pth, name_seg)
            seg_fold = pd.read_pickle(datafile)  # list
            seg = seg_fold[int(count)][0]  # select segmentation of the patient
            total_mask = total_mask + seg
        final_mask = total_mask/len(folds)  # The maximum should be 1 and the minimum 0
        # Save patient consolidated mask as numpy array
        patient_name = pid + '.npy'
        #outpth = r'/scratch_net/biwidl311/Cristina_Almagro/pytorch3D/detection_unet/test_pred_seg'
        outpth_pid = os.path.join(outpth, patient_name)
        np.save(outpth_pid, final_mask)
