...
# Contributions: besides adapting the algorithm for the MRI dataset of Biomedical Imaging Computing lab, I fixed this
# function so the algorithm can work with single class image segmentation (e.g.  background + metastasis in our case).
# Prior to this adaptation the algorithm required two classes besides the background for functioning.
#######################################################################################################################

'''
Example Data Loader for the my data set. This dataloader expects preprocessed data in .npy or .npz files per patient and
...
def get_train_generators(cf, logger):
    ...
    train_pids = all_pids_list[:int(2.5 * cf.n_train_val_data // 3)]  # Select training pid (select first x numbers specified by the x quantity)
    val_pids = all_pids_list[int(np.ceil(2.5 * cf.n_train_val_data // 3)):cf.n_train_val_data]  # Select testing pid # select the rest of ids for validation
...
def get_test_generator(cf, logger):
    ...
    batch_gen['n_test'] = 25 #29 #  #54  # do 2 for debugging
    return batch_gen
...
def load_dataset(cf, logger, subset_ixs=None, pp_data_path=None, pp_name=None):
    ...
    :return: data: dictionary with one entry per patient. Each entry is a dictionary containing respective meta-info as well as paths to the preprocessed
    numpy arrays to be loaded during batch-generation
    """
    if pp_data_path is None:
        pp_data_path = cf.pp_data_path  # Folder with the preprocessed images for training
    if pp_name is None:
        pp_name = cf.pp_name
    if cf.server_env:
        copy_data = True
    ...
    p_df = pd.read_pickle(os.path.join(pp_data_path, cf.input_df_name))   # It reads the info_df.pckl file located in the train or test folder
    #print(p_df)
    ...
    if subset_ixs is not None:  # This is none in both training & testing
    ...
    patients_name = p_df.patient_name.tolist()
    ...
    for ix, pid in enumerate(pids):
        ...
        # In the case I want to include class_target = 1 for all entries
        targets = 0
        data[pid] = {'data': imgs[ix], 'seg': segs[ix], 'pid': pid, 'class_target': targets, 'patient_name': patients_name[ix]}
...
def create_data_gen_pipeline(patient_data, cf, is_training=True):
    ...
    my_transforms = []
    if is_training:
        mirror_transform = Mirror(axes=np.arange(cf.dim))  # I think is axes 0 & 1
        ...
        my_transforms.append(CenterCropTransform(crop_size=cf.patch_size[:cf.dim])) # Just take the dimensions of patch size: [128 128 64]
...
class BatchGenerator(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        if self.cf.head_classes > 2:  # In my case is 2 so the code will not run through this
        ...
        for b in batch_ixs:
            ...
            data = all_data[0]  # Image with dimensions 384 x 384 x 272
            data = data[np.newaxis]  # Image with dimensions 1 x 384 x 384 x 272 (I have not flipped the x and y dimensions)
            seg = all_data[1].astype('uint8')
            batch_pids.append(str(patient['pid']))
            batch_targets.append(patient['class_target'])  # May be remove this ?

            if self.cf.dim == 2:  # This part does not have to be considered
            ...
            if np.any([data.shape[dim + 1] < ps for dim, ps in enumerate(self.cf.pre_crop_size)]):
                print(" Padding was required while it should not be!")
            ...
            if len(crop_dims) > 0:
                fg_prob_sample = np.random.rand(1)  # It outputs a number between 0 & 1
                ...
                if fg_prob_sample < self.p_fg and np.sum(seg) > 0:
                    seg_ixs = np.argwhere(seg == np.random.choice(np.unique(seg)[1:], 1))   # Just locations where the mask is 1
        ...
        #data = np.array(batch_data).astype(np.float16)  # This one used in PET-CT
        seg = np.array(batch_segs).astype(np.uint8)

        #class_targets = 1 if np.sum(seg) > 0 else 0
        #batch_targets.append(class_targets) # If it is 0 it will contain a metastases. If it is -1 it won't contain a metastasis.
...
class PatientBatchIterator(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        patient['class_target'] = patient['class_target']


        a = patient['class_target']
        ...
        batch_class_targets = []
        batch_class_targets.append(np.array(patient['class_target']))

        #print("Initial batch_class_targets are...", batch_class_targets)
        ...
        if np.any([data.shape[dim + 1] < ps for dim, ps in enumerate(self.patch_size)]):
            print(" Test image smaller than training patches. Something failed.")
        ...
        if self.cf.dim == 3 or self.cf.merge_2D_to_3D_preds:
            ...
            #batch_3D = {'data': out_data, 'seg': out_seg,'pid': pid}
        ...
        if self.cf.dim == 2:
            ...
            #batch_2D = {'data': out_data, 'seg': out_seg, 'pid': pid}
        ...
        if np.any([data.shape[dim + 1] > self.patch_size[dim] for dim in range(3)]):
            ...
            a = patient_batch['roi_labels']
            a = np.array(a)
            batch_class_targets = [[0] * a.shape[1] for i in range(96)]
...
## Create a main file & perform checking of code: This allows you to get familiarized on how the data will be loaded
#during training and testing

## MAIN CODE FOR CHECKING CREATING THE TRAINING GENERATOR
...
if __name__=="__main__":
    ...
    # Load individual masks for each metastasis
    mydata_roim = train_batch['roi_masks']
    print(mydata_roim.shape) #(8,)


    # Load ground truth bounding box for each metastasis
    mydata_bb = train_batch['bb_target']
    print("print my data bb...")
    print(mydata_bb)
    #print(train_batch.items())

    # Load segmentations masks containing all metastases
    mydata_seg = train_batch['seg']
    print(mydata_seg.shape)
    matplotlib.use('TkAgg')
    import matplotlib.pyplot as plt
    plt.imshow(mydata_seg[1, 0, :, :, 10])
    plt.show()

    # PATIENT ID
    mydata_id = train_batch['pid']

    # ROI LABELS
    mydata_roil = train_batch['roi_labels']
    print(mydata_roil)
    print(type(mydata_roil))
    ...
    for kk in range(8):
        ...
        print("Patient", kk)
        print("has the metastases...", np.max(seg_labelled))


# MAIN CODE FOR CHECKING THE TEST GENERATOR
...
if __name__ == "__main__":
    ...
    # 1: data
    mydata = test_batch['data']
    print(mydata.shape)  # (96, 1, 128, 128, 64)
    matplotlib.use('TkAgg')
    import matplotlib.pyplot as plt
    plt.imshow(mydata[1, 0, :, :, 10], cmap='gray')
    plt.show()

    # 2: seg
    mydata_seg = test_batch['seg']
    print(mydata_seg.shape)  # (96, 1, 128, 128, 64)
    matplotlib.use('TkAgg')
    import matplotlib.pyplot as plt

    plt.imshow(mydata_seg[1, 0, :, :, 10], cmap='gray')
    plt.show()

    # 3: pid
    pid = test_batch['pid']
    print(pid)  # 1 (first patient)

    # 4: patch_crop_coordinates
    patch_crop_coordinates = test_batch['patch_crop_coords']
    print(patch_crop_coordinates)
    print(patch_crop_coordinates.shape)  # (96,6)

    # 5: patient_bb_target
    patient_bb = test_batch['patient_bb_target']
    print(patient_bb.shape)

    # 6: patient_roi_labels
    patient_roil = test_batch['patient_roi_labels']
    print(patient_roil.shape)  # 2
    print(patient_roil)
#

    # 7: Original_img_shape
    img_shape = test_batch['original_img_shape']
    #img_shape: (1,1,384,384,272)

    # 8: bb_target
    bb_target = test_batch['bb_target']
    print(bb_target.shape)  # 96 (as many as patches)
    print(bb_target)

    # 9: roi_masks
    roi_masks = test_batch['roi_masks']
    print(roi_masks.shape)  # (96, 1, 1, 128, 128, 64)
    print(roi_masks)

    # 10: roi_labels
    roi_labels = test_batch['roi_labels']
    print(roi_labels.shape)  # 96 (as many as patches)
    print(roi_labels)
#
