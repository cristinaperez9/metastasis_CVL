...
def compute_mrcnn_bbox_loss(mrcnn_target_deltas, mrcnn_pred_deltas, target_class_ids):
    ...
    :return: loss: torch 1D tensor.
    """
    # print("It works. Visualizing target_class_ids...")
    # a = target_class_ids.detach().cpu()  # Move tensor to CPU
    # n = a.numpy()  # Convert torch tensor to numpy
    # print(n)  # Print numpy array
...
class net(nn.Module):
    ...
    def train_forward(self, batch, is_validation=False):
        ...
        results_dict['monitor_values'] = {'loss': loss.item(), 'class_loss': mrcnn_class_loss.item(), 'bbox_loss': mrcnn_bbox_loss.item(), 'mask_loss': mrcnn_mask_loss.item(), 'rpn_class_loss': rpn_class_loss.item(), 'rpn_bbox_loss': rpn_bbox_loss.item()}
