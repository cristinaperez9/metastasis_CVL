...
# ---------------------------------------------------------------------------------------------------------------
# Contributions: created code of the consolidation of segmentation masks through averaging over several epochs,
#  test time augmentations and overlapping patches
# The initial code was intended for detection and this adaptation allows segmentation.
#################################################################################################################
...
class Predictor:
    ...
    def predict_test_set(self, batch_gen, net,return_results=True):
        """
            ...
            with torch.no_grad():
                for _ in range(batch_gen['n_test']):
                    ...
                    if rank_ix == 0:
                        ...
                        dict_of_patient_results[batch['pid']]['results_list'] = {}
                        dict_of_patient_results[batch['pid']]['results_list']['boxes'] = []  # ADDED BY ME
                        dict_of_patient_results[batch['pid']]['results_list']['seg_preds'] = []  # ADDED BY ME

                    # call prediction pipeline and store results in dict.
                    results_dict = self.predict_patient(batch)

                    # if np.max(results_dict['seg_preds']) > 0:
                    #     print("Segmentation mask is not empty!")
                    # else:
                    #     print("EMPTY!")
                    ...
                    dict_of_patient_results0 = dict_of_patient_results
                    #dict_of_patient_results0[batch['pid']]['results_list'].append(results_dict['boxes'])
                    dict_of_patient_results[batch['pid']]['results_list']['boxes'].append(results_dict['boxes']) # ADDED BY ME
                    dict_of_patient_results[batch['pid']]['results_list']['seg_preds'].append(results_dict['seg_preds'])   # ADDED BY ME

                    #print("Printing dict_of_patient_results...")
                    #print(dict_of_patient_results)

                    #a = dict_of_patient_results[batch['pid']]['results_list'][1]
                    #print(type(a)) #numpy array
                    #print(a.shape) #for first patient is (1,4,320,320) the 4 probably because there are 4 double predictions for diff labels

        self.logger.info('finished predicting test set. starting post-processing of predictions.')
        list_of_results_per_patient = []  # it only contains the boxes
        ...
        for pid, p_dict in dict_of_patient_results.items():  # for pid: loop over all patients,

            print("Printing pid", pid)

            tmp_ens_list = p_dict['results_list']  # this is a dictionary now but it was a list before
            results_dict = {}

            results_dict['boxes'] = [[item for d in tmp_ens_list['boxes'] for item in d[batch_instance]]
                                     for batch_instance in range(len(tmp_ens_list['boxes'][0]))]


            results_dict['seg_preds'] = np.array([[item for d in tmp_ens_list['seg_preds'] for item in d[batch_instance]] for batch_instance in range(len(tmp_ens_list['boxes'][0]))])


            ## TODO return for instance segmentation:
            a = np.mean(results_dict['seg_preds'], 1)
            results_dict['seg_preds'] = np.mean(results_dict['seg_preds'], 1)[:, None]

            for b in range(p_dict['patient_bb_target'].shape[0]):  # number of metastases per patient
            ...
            list_of_results_per_patient_seg.append([results_dict['seg_preds'], pid])

        # save out raw predictions (only boxes as original code).
        ap = self.cf.ap_match_ious[0]
        name = 'raw_pred_boxes_hold_out_list_' + str(ap) + '.pickle'
        out_string = name if self.cf.hold_out_test_set else 'raw_pred_boxes_list'
        ...
        name = 'raw_pred_boxes_&_seg_hold_out_list_' + str(ap) + '.pickle'
        out_string = name if self.cf.hold_out_test_set else 'raw_pred_boxes_list'
        with open(os.path.join(self.cf.fold_dir, '{}.pickle'.format(out_string)), 'wb') as handle:
            pickle.dump(list_of_results_per_patient_seg, handle)
        ...
        if return_results:
            ...
            # merge 2D boxes to 3D cubes. (if model predicts 2D but evaluation is run in 3D) ######## NOT IN MY MODEL ##########
            if self.cf.merge_2D_to_3D_preds:
    ...
    def load_saved_predictions(self, apply_wbc=False):
        ...
        # if hold out test set was predicted, aggregate predictions of all trained models
        # corresponding to all CV-folds and flatten them.
            ...
            for fold in self.cf.folds:
                ...
                ap = self.cf.ap_match_ious[0]
                name = 'raw_pred_boxes_hold_out_list_' + str(ap) + '.pickle'
                with open(os.path.join(fold_dir, name), 'rb') as handle:
