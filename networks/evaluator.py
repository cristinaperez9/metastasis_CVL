...
##################################################################################################################
# Segmentation evaluation added by: Cristina Almagro Perez, 2022. ETH University.
# ----------------------------------------------------------------------------------------------------------------
# Contributions:
# Function: compute_dice_patient
# Function: compute_iou_patient
# Function: compute_dice_batch
##################################################################################################################
...
class Evaluator():
    ...
    def return_metrics(self, monitor_metrics=None):
        ...
        for cl in list(self.cf.class_dict.keys()):
            ...
            for score_level in self.cf.report_score_level:
                ...
                if self.cf.scan_det_thresh:
                    conf_threshs = list(np.arange(0.8, 1, 0.01)) #0.9
        ...
        # in small data sets, values of model_selection_criterion can be identical across epochs, which breaks the
        # ranking of model_selector. Thus, perturbe identical values by a neglectibale random term.
    ...
    def score_test_df(self, internal_df=True):
        ...
        if internal_df:
            ...
            nm = 'results_ap_' + str(ap) + '.txt'
            with open(os.path.join(self.cf.exp_dir, nm), 'a') as handle:
...
def compute_dice_batch(gt_masks, pred_masks):

    batch_size = gt_masks.shape[0]  # the first dimension is the batch dimension

    # Check if masks are 2D or 3D
    shape_masks = gt_masks.shape
    if len(shape_masks) == 4:
        dim = 2
    else:
        dim = 3
    ...
    for i in range(batch_size):
        ...
        # Check that at least there is a ground truth metastasis or prediction (check not both masks are 0)
        val = np.sum(gt) + np.sum(pred)
        if val > 0:
            count = count + 1
            # Calculate DICE
            ...
            dice_value = n / d
            #print("Dice value of current patch: ", dice_value)
            dice_batch = dice_batch + dice_value
    dice_batch = dice_batch / count
    print("Dice value of current batch: ", dice_batch)
    return dice_batch
