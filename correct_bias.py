...
def myN4(inputImage,pth):
    print("N4 bias correction runs.")
    maskImage = sitk.OtsuThreshold(inputImage,0,1,200)
    # Code to save the mask image
    filename = os.path.join(pth, 'mask_for_bias_correction.nii.gz')
    sitk.WriteImage(maskImage,filename)

    image = sitk.Shrink(inputImage,[4, 4, 1])
    maskImage = sitk.Shrink(maskImage,[4, 4, 1])
    ...
    filename = os.path.join(pth, 'reg_img_cropped_bias_correc.nii.gz')
    sitk.WriteImage(corrected_image_full_resolution,filename)
    print("Finished N4 Bias Field Correction.....")

#####################################################################################
# SPECIFY THE FOLLORING:
one_image = True # Test bias-correction code for one image
visualize_one_image = True # Visualize example image before and after bias correction
...
if one_image:
    ...
    myN4(inputImage,pth) #It will write the bias corrected image

if vusualize_one_image:
...
if all_images:
    pth00 = r'/usr/bmicnas01/data-biwi-01/bmicdatasets-originals/Originals/USZ/metastases_segmentation/Processed/Registered/'
    outpth00 = r'/usr/bmicnas01/data-biwi-01/bmicdatasets/Processed/Cristina_Almagro/'
    ...
    for imyfiles in myfiles:
        ...
        for ipatients in patients:
            ...
            if DS ==-1:
                inputImage = sitk.ReadImage(filename, sitk.sitkFloat32)
                myN4(inputImage,outpth) # It will write the bias corrected image and the mask brain-background used